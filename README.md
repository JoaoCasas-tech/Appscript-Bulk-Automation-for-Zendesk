...
This project contains a Google Apps Script automation that creates Zendesk tickets in bulk from a CSV file imported into Google Sheets.

The script reads customer communication data from a spreadsheet, sends the records to Zendesk using the bulk ticket creation endpoint, and writes the result of each processed row into an audit log.

---

## Why this solution uses bulk creation

A simpler version of this project could read the CSV row by row and send one API request per customer.
...
That approach is easier to implement, but it does not scale well.
...
This makes the process more efficient, scalable, and safer for larger files.
...
The second tab is the audit log tab.
...
The second tab must be named exactly:
...
```text
bulk_<run_id>_row_<source_row_number>
```
...
```text
zendesk_bulk_ticket_automation.gs
```
...
The person running the script must provide:
...
This project is designed to keep sensitive information outside of the source code.

Zendesk credentials should be stored only in Apps Script Properties.

The repository should not contain:

```text
real Zendesk API tokens
real customer data
private CSV files
production audit logs with customer information
```

Before making this repository public, review all files and make sure no confidential data or credentials were committed.

---

## Suggested use case

This automation can be used for operational workflows where a team needs to create Zendesk tickets in bulk from structured customer communication data.
...
support follow-ups
policy communication tasks
operational outreach campaigns
```
...
This project is a portfolio-ready automation example.

It is intended to demonstrate practical experience with customer operations tooling, support automation, API integrations, and scalable workflow design.